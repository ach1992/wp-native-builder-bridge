...
WP Native Builder Bridge
      ...
      +-- Media / taxonomies / navigation
      +-- Astra / Astra Pro when present
      +-- Gravity Forms when present
      +-- Code Snippets Pro when present and supported through a public API
      +-- WordPress plugin/theme/user administration when explicitly enabled
```

### Required platform baseline

- WordPress 6.9+ because the Abilities API is part of WordPress core from 6.9.
- Official `WordPress/mcp-adapter` installed and active for MCP exposure.
...
Do not bundle a private copy of MCP Adapter. Detect it at runtime and show a concise admin notice/setup instruction when it is unavailable. If the official adapter later becomes directly resolvable through WordPress plugin dependencies, the project may adopt that mechanism without changing this architecture.
...
5. **Capability enforcement** — every ability checks the current WordPress user's required capability.
6. **Admin-controlled exposure** — powerful ability groups can be disabled entirely.
...
9. **Extensible integrations** — optional integrations register abilities only when the target plugin/theme is present and supported.
10. **Fast delivery** — avoid architecture that does not materially improve the first useful release.
...
| Ability permission callback | Require the appropriate WordPress capability and enabled access group |
...
The first usable release should cover the following functional surface. Implementation may combine closely related operations when one typed operation with an explicit `action` field is clearer and safer than many tiny tools.
...
Support WordPress posts, pages, and registered editable custom post types through normal WordPress APIs:
...
Use public WordPress APIs and feature detection.

### 7.7 Astra / Astra Pro

When Astra/Astra Pro is active, expose only operations that can be implemented through supported/public WordPress/Astra interfaces rather than undocumented database internals.
...
If a specific Astra feature lacks a stable public API, report it as unsupported instead of silently binding the bridge to fragile private internals. A generic WordPress custom-post-type operation may be used only when the underlying registered type and capabilities make that safe and supportable.

### 7.8 Gravity Forms

When Gravity Forms is active and its public API is available, support through `GFAPI` or another documented public interface:
...
When Code Snippets Pro is active, integrate only through its current documented public API/REST/WP-CLI/programmatic interfaces that are suitable for third-party use.
...
When `Code & Extensions` is enabled and the current WordPress user has the required capabilities, support normal WordPress lifecycle operations through WordPress APIs:
...
When `Site Configuration` is enabled, provide typed operations for the site settings genuinely useful to site building, such as:
...
When `Users & Destructive` is enabled and WordPress capabilities allow it, support bounded user/role administration needed for site management:
...
- perform capability checks in every ability permission callback;
...
Treat the official MCP Adapter as the transport implementation.
...
- Core WordPress functionality must not require Astra, Gravity Forms, or Code Snippets Pro.
- Optional integrations activate only when their dependency is available and a supported API exists.
...
Do not create a framework of abstractions before repeated behavior proves it useful. Group abilities/integrations enough to keep files understandable and testable.
...
9. Create/update/activate a Code Snippet when a stable Code Snippets integration is available.
...
- WordPress capability checks and admin switches reliably deny disabled/unauthorized actions;
...
Bootstrap + dependency + permissions
  -> Core inspection/content/blocks/media/navigation
...
The intended result is a small powerful bridge, not a large platform. Add complexity only when a real ability or verified interoperability requirement needs it.
